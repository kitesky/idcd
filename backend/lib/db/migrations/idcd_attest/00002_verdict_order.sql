...
-- D1: owner_id 指向 idcd_main.users(id), ext_order_id 指向 idcd_main 09 order
...
CREATE TABLE IF NOT EXISTS idcd_attest.verdict_order (
  ...
  price_paid_cny          NUMERIC(10,2),                           -- 实收(聚合支付扣费后)
  ext_order_id            TEXT,                                    -- D1: 关联 idcd_main 09 order, 不写 FK
  refund_reason           TEXT,
  refund_attempt_count    INT NOT NULL DEFAULT 0,                  -- D5: refund retry queue 累计次数
  refund_last_error       TEXT,                                    -- D5: 最后一次 refund 失败原因(聚合支付风控 / 网络 等)
