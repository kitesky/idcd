...
| `cert-worker` | `backend/apps/cert-svc/cmd/worker` 子命令 | 新 | ACME orchestrator，消费 Redis Stream 跑状态机 |
| `cert-renewer` | `backend/apps/cert-svc/cmd/renewer` 子命令 | 新 | cron 调度续期，复用 worker 队列 |
| `lib/cert-ca` | `backend/lib/cert/ca` | 新 | CA 适配层（封装 `go-acme/lego`） |
| `lib/cert-dns` | `backend/lib/cert/dns` | 新 | DNS provider 适配（复用 lego 的 provider 集） |
| `lib/cert-vault` | `backend/lib/cert/vault` | 新 | 私钥 / DNS 凭据加密存取，对接 KMS |
| `apps/web` | `frontend/app/app/cert/*` | 扩展 | 前端路由组，复用 `(app)` Sidebar 布局 |
...
| `apps/admin`（在 `frontend/app/admin/cert/*`） | 扩展 | — | 管理面板 |
...
| 限流 | `lib/ratelimit` |
| 日志 / Metric / Trace | `backend/lib/shared/obs` |
| 邮件发送 | `apps/notifier` |
| 配置中心 | `backend/lib/shared/config` |
...
> **MPIC 影响**：自 2025 起 LE / GTS 等主流 CA 都已启用 Multi-Perspective Issuance Corroboration，从多个 vantage point 各自查询 DNS 验证 challenge。这意味着我们写完 TXT 必须等到**全球权威 NS 都生效**，而不仅本地 resolver 看到。`backend/lib/cert/dns` 的传播检查必须直接 dig 域名的 authoritative NS（不查 8.8.8.8 这类公共 resolver 缓存）。
...
- `backend/lib/cert/*` 行覆盖 ≥ 90%；纯算函数 100%
...
走 idcd 现有 migration 工具（`backend/lib/db/migrate`），新建 `migrations/cert/*.sql`。lint：跨 schema FK 检测（scripts/lint-cross-schema-fk.sh 已存在）。
...
| 周 | 交付 |
|---|---|
| W1 | DB schema + migration；`apps/cert-svc` 骨架（HTTP + worker 子命令）；`backend/lib/cert/ca` Let's Encrypt 适配；`backend/lib/cert/vault` env-master-key 实现 |
| W2 | ACME 完整状态机 + WAL；CSR 生成；Pebble 集成测试 |
| W3 | `backend/lib/cert/dns` Cloudflare provider + 手动模式；DNS 凭据加解密 + UI |
...
2. 派单：W1 任务可拆 3 个并行 worktree
   ...
   - WT-B：`backend/lib/cert/ca` LE adapter + Pebble 测试（**接口设计必须考虑付费 CA 适配，详 §20.3**）
   - WT-C：`backend/lib/cert/vault` env-master-key 实现 + 单元测试
...
| 域名所有权验证 (DCV) | ACME challenge | DNS-01 / HTTP-01 / Email | 共用 `backend/lib/cert/dns` 写 TXT；Email DCV 是新增 |
...
```go
// backend/lib/cert/ca/ca.go (S1 创建)
type Tier string
const (
...
- [ ] `backend/lib/cert/ca` 接口分为 `CA` 基接口 + `AcmeCA` 子接口
