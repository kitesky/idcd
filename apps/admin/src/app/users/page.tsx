...
export default function UsersPage() {
  ...
  const load = useCallback(() => {
    ...
    if (q) params.set("q", q)
    fetch(`${API_BASE}/internal/admin/users?${params}`, {
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <input
          ...
          className="flex h-9 w-64 rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
        />
            ...
            <TableBody>
              {data?.users.map((u) => (
                <TableRow key={u.id}>
                  ...
                  <TableCell>
                    <Badge variant={planVariant[u.plan] ?? "outline"}>{u.plan}</Badge>
                  </TableCell>
