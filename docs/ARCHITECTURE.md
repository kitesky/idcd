...
```text
idcd/
├── apps/                          # 9 Go services + 4 Workers + 1 Next.js app
│   ├── web/                       # Next.js 主站(idcd.com)— SSG 工具页 + CSR 控制台 + admin + status + docs
│   │
...
│   ├── cli/                       # Go CLI 工具(idcd 命令行)
│   │
│   ├── attest/                    # Go Attestation 服务(api + generator + verifier + refund-worker)
│   │
│   └── mcp/                       # Go MCP Server(mcp.idcd.com,业务 stateless + SSE stateful)
│
├── lib/                           # Go 共享库(Go module)
│   ├── auth/                      # JWT / session / API key / MCP token 共享逻辑
│   ├── attest/                    # 签名 / TSA / PDF / WAL record
│   │   ├── sign/                  # KMS 抽象层(D4 idempotency token 必传)
│   │   ├── tsa/                   # RFC3161 TSA client(主备三家:DigiCert / GlobalSign / NTSC)
│   │   ├── pdfsign/               # PDF PAdES 签名
│   │   └── record/                # attestation_record WAL 逻辑
...
│   │   ├── docker-compose.attest.yml        # attest server/generator(独立栈)
│   │   ├── docker-compose.attest-verify.yml # D6 独立 service + 独立 subnet
│   │   ├── docker-compose.mcp.yml           # mcp + docs-mcp
...
├── pnpm-workspace.yaml
├── go.work                        # Go workspace(9 apps + lib/* + packages/* 各自 module)
...
2. **`apps/` 与 `lib/` / `packages/` 物理分离**:`apps/` 是可部署单元(有 Dockerfile + main.go);`lib/` 是被 import 的 Go 共享库(无 main);`packages/` 是 SDK 发布包。
3. **`attest/*` / `mcp` 独立 docker compose 栈**:不与 core 共栈,K1 三栈 sub-product 阵型的实施落地。
...
| **mcp** | mcp.idcd.com :8087 | 多实例 + **LB sticky session**(D13)| PG + Redis Pub-Sub | 业务 stateless,SSE stateful;10k 并发 SSE / 实例 |
| **cli** | (本地) | 本地二进制 | — | 开发者 CLI 工具(idcd 命令行) |
...
| **attest/generator** (Verdict Generator) | 独立 docker container | Redis Stream `verdict_generation_queue` | WAL on `attestation_record`(D4);每 step 前查 success 跳过;KMS sign + idempotency token |
| **attest/verifier** (Self-Verify Worker) | **独立 docker container + 独立 VPC subnet + 独立 KMS 客户端实例**(D6) | 内部 cron(每报告 1 次 + 每日抽样 10 份) | 仅调用 attest.idcd.com/verify 公开接口,**不复用** attest/generator 任何代码 / 配置 / 缓存 |
| **attest/refund-worker** (Refund Worker) | 独立 docker container | retry queue(5min → 30min) | D5:30min 失败强制道歉邮箱 + `verdict_order.status=refund_failed` + P0 告警 + admin dashboard 待处理 |

### 2.3 前端 Next.js app(1 个)

| App | 域名 | 渲染策略 | 备注 |
|---|---|---|---|
| **web** | idcd.com | 工具页 SSG + 控制台 CSR + admin CSR + status ISR + docs SSG | shadcn/ui + Tailwind v4 + ECharts;包含 admin、status、docs 等所有前端功能 |
...
- 网络拓扑展示两个 subnet
- 运行时 strace / OpenTelemetry trace 显示 attest/verifier 仅调 verify HTTP 接口,无内部 RPC 调用
...
**为什么**:`mcp` 的业务逻辑 stateless(可水平扩展),但 `/v1/agent-obs/events` 是 long-lived SSE 连接(stateful)。
...
| **业务状态隔离** | 鉴权 / 计量 / config 状态全在 PG + Redis;任一 mcp 实例可处理任一**非 SSE**请求。 |
...
- **`attest/*` services 独立 release**(不与 core 耦合):attest-api / attest/generator / attest/verifier / attest/refund-worker 共享 release tag(同步部署)
...
- [ ] 跑通 `make dev-up` + 浏览 idcd.com(包含 admin、status、docs 等所有前端功能)
- [ ] 通读 OVERVIEW.md + 本文档 §1-§3
- [ ] 完成第一个 PR:加一个 trivial endpoint(如 `/api/health/deep`)
- [ ] 跑通 attest/generator 完整链路(用 staging 测试 KMS,看到 PDF 输出)
- [ ] 跑通 mcp 本地(Claude Code 连接 `http://localhost:8087/mcp`)
...
| **Traces** | OpenTelemetry + Tempo | 端到端追踪;`attest/generator` 每 step 必加 span |
