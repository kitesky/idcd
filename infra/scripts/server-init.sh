...
GATEWAY_PORT="${GATEWAY_PORT:-8443}"  # apps/gateway agent mTLS WSS
...
echo "     模板: infra/docker/cert-svc.env.example"
echo "     ⚠️ 严禁复用 dev 全零 MASTER_KEY"
echo "     生成: openssl rand -base64 32"
echo ""
echo "  4) 把 nginx 配置 + TLS 证书放好"
echo "     /opt/idcd/nginx/nginx.conf            (从 infra/nginx/nginx.conf 拷贝)"
