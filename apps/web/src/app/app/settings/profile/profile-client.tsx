...
function LanguageSwitcher() {
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor="language-switcher">{t("profile.language")}</Label>
      ...
      <Select value={locale} onValueChange={(v) => handleChange(v as "zh" | "en")}>
        <SelectTrigger id="language-switcher" className="w-48" data-testid="language-switcher">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
...
export function ProfileClient() {
  ...
  // Release the last preview blob URL when the component unmounts so the
  // blob ID doesn't sit in the browser's blob store forever. Each new
  // upload also revokes the previous URL inline (see handleFileChange).
  useEffect(() => {
    return () => {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Show local preview immediately. Revoke the previous preview blob first
    // so repeated uploads don't leak blob URLs in the browser blob store.
    if (avatarPreview) URL.revokeObjectURL(avatarPreview)
