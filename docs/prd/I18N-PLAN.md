...
| 资产 | 文件 | 说明 |
|---|---|---|
| 路由 / middleware | `frontend/src/proxy.ts` | `/en/*` rewrite + 注入 `x-locale: en` header |
| 翻译目录 | `frontend/src/i18n/messages/{zh,en}/*.json` | 16 namespace，需重命名 `zh` → `cn` |
| 配置 | `frontend/src/i18n/routing.ts` | locales=['zh','en']，需改 ['cn','en'] |
| 客户端 hook | `useTranslations()` | next-intl 3.26.0 已接入 |
| 服务端 helper | `frontend/src/i18n/getT.ts` | 服务端组件按 namespace 加载 |
| 语言切换 UI | `frontend/src/components/nav.tsx:304-391` LangToggle | URL 前缀 + cookie |
| Tool SEO 元数据 | `frontend/src/i18n/en-tools-meta.ts` | 21 工具 title/description/schemaName |
...
**自写极简 catalog**（~150 行），位于 `backend/apps/api/internal/i18n/`：
...
backend/apps/api/internal/errcode/
├── codes.go                                       # const 块，全量错误码
└── http.go                                        # HTTP status mapping

backend/apps/notifier/internal/i18n/
├── registry.go                                    # 共享同一份 registry
└── catalog.go                                     # 复用 API 的 catalog 逻辑

backend/apps/notifier/internal/template/
...
- `config/locales.json`
- `frontend/src/i18n/registry.ts`（generate from JSON）
- `backend/apps/api/internal/i18n/registry.go`
- `backend/apps/notifier/internal/i18n/registry.go`
...
#### 1.1 Catalog 实现
新建 `backend/apps/api/internal/i18n/catalog.go` + ICU plural 支持 + 单元测试。

#### 1.2 Locale Middleware
插入 Chi 路由链路开头，解析后写入 `ctx.Locale`。

#### 1.3 错误码集中
`backend/apps/api/internal/errcode/codes.go`：
...
**邮件全维度国际化**：
- ✅ Subject（每个模板配套 `subject.cn` / `subject.en` key 在 `backend/apps/notifier/internal/i18n/messages/{locale}/email.json`）
...
#### 3.3 错误码 key 集合校验
CI lint：扫 `backend/apps/api/internal/errcode/codes.go` 的常量名，断言每个 errcode 都在 `frontend/src/i18n/messages/{locale}/errors.json` 中存在。
...
重点文件：
- `frontend/src/components/layout/nav-user.tsx`（行 61-125 全部硬编码）
- `frontend/src/app/app/monitors/*`
- `frontend/src/app/app/alerts/*`
- `frontend/src/app/app/billing/*`
- `frontend/src/app/app/settings/*`
- `frontend/src/app/app/dashboard/*`
- `frontend/src/app/app/status-pages/*`
- `frontend/src/app/app/incidents/*`
...
#### sitemap.xml
`frontend/src/app/sitemap.ts` 用 registry 自动生成：每个 URL 输出 N 份 locale 版本（带 `xhtml:link`）。
...
**批次 4a（用户后台，可并发）**：
- Agent A1：`frontend/src/app/app/monitors/*` + `frontend/src/components/layout/nav-user.tsx`
- Agent A2：`frontend/src/app/app/alerts/*`
- Agent A3：`frontend/src/app/app/settings/*` + `frontend/src/app/app/billing/*` + `frontend/src/app/app/usage/*`
- Agent A4：`frontend/src/app/app/dashboard/*` + `frontend/src/app/app/incidents/*` + `frontend/src/app/app/status-pages/*`

**批次 4c（公开页，可并发）**：
- Agent C1：`frontend/src/components/nav.tsx` + `frontend/src/components/footer.tsx`
- Agent C2：`frontend/src/app/(public)/tools/*` + `frontend/src/lib/tool-functions.ts`
- Agent C3：`frontend/src/app/(public)/{about,aup,privacy,agent,transparency}/*`
- Agent C4：`frontend/src/app/(public)/{leaderboard,nodes,pricing,home}/*`
...
2. **复制 messages 目录**：
   ```bash
   cp -r frontend/src/i18n/messages/cn frontend/src/i18n/messages/ja
   cp -r backend/apps/api/internal/i18n/messages/cn backend/apps/api/internal/i18n/messages/ja
   ```

3. **复制邮件模板**：
   ```bash
   for f in backend/apps/notifier/internal/template/*.cn.html; do
     cp "$f" "${f/.cn./.ja.}"
   done
   ```
...
4. **复制文档 MDX**：
   ```bash
   for d in frontend/src/content/docs/*/; do
...
backend/apps/notifier/internal/
├── i18n/                                              # 复用 API catalog
├── template/
...
backend/lib/db/migrations/idcd_main/
