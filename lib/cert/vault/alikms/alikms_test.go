...
// ---------- mockKMSClient ----------
//
// 用一个本地固定 32 字节"假主密钥" + AES-256-GCM 模拟阿里云 KMS：
...
//
// 这样 mock 的语义和真 KMS 一致：明文 DEK 离开 mock 后就只剩密文版本，
// 调用 Decrypt 才能还原。
...
func TestNew_MissingFields(t *testing.T) {
	cases := []struct {
		...
		{"empty all", Config{}},
		{"empty RegionID", Config{AccessKeyID: "a", AccessKeySecret: "b", KeyID: "k"}},
		{"empty AccessKeyID", Config{RegionID: "cn-hangzhou", AccessKeySecret: "b", KeyID: "k"}},
		{"empty AccessKeySecret", Config{RegionID: "cn-hangzhou", AccessKeyID: "a", KeyID: "k"}},
		{"empty KeyID", Config{RegionID: "cn-hangzhou", AccessKeyID: "a", AccessKeySecret: "b"}},
	}
	for _, c := range cases {
...
func TestDecryptKey_TamperedEncryptedDEK(t *testing.T) {
	...
	// 翻转 Ciphertext[5]，落在 encDEK 区域内（前 4 字节是长度前缀，第 5 字节是 encDEK 的第 2 字节）
...
func TestDecryptKey_HeaderLenExceedsBuf(t *testing.T) {
	...
	_, v := mustVault(t)
	ct := make([]byte, 4)
	// 声明 100 字节 encDEK，但实际 buf 只有 4 字节 header
...
func TestDecrypt_KMSReturnsBadDEKLen(t *testing.T) {
	ctx := context.Background()
	m, v := mustVault(t)
	// 先正常加密一次（用真随机 DEK），然后 hack Decrypt 返回错长度
...
func TestErrorMessages_HaveContext(t *testing.T) {
	...
	// 至少要带上字段名提示
	if !bytes.Contains([]byte(err.Error()), []byte("RegionID")) {
		t.Fatalf("err lacks field name: %v", err)
	}

	cases := []Config{
		{RegionID: "cn-hangzhou"},
		{RegionID: "cn-hangzhou", AccessKeyID: "a"},
		{RegionID: "cn-hangzhou", AccessKeyID: "a", AccessKeySecret: "b"},
	}
	wantFields := []string{"AccessKeyID", "AccessKeySecret", "KeyID"}
	for i, c := range cases {
		_, err := New(c)
		if err == nil {
...
// 这是唯一不依赖真实阿里云就能跑的路径。其余路径需要集成测试，不在此覆盖。
...
func TestRealKMSClient_GenerateDataKey_OK(t *testing.T) {
	...
	r := &realKMSClient{
		keyID: "alias/test",
		sdkGen: func(req *kmssdk.GenerateDataKeyRequest) (*kmssdk.GenerateDataKeyResponse, error) {
			if req.KeyId == nil || *req.KeyId != "alias/test" {
				t.Errorf("KeyId = %v", req.KeyId)
			}
			if req.KeySpec == nil || *req.KeySpec != "AES_256" {
				t.Errorf("KeySpec = %v", req.KeySpec)
			}
			return &kmssdk.GenerateDataKeyResponse{
				Body: &kmssdk.GenerateDataKeyResponseBody{
					Plaintext:      strPtr(base64.StdEncoding.EncodeToString(dek)),
					CiphertextBlob: strPtr(base64.StdEncoding.EncodeToString(enc)),
				},
			}, nil
		},
	}
...
func TestRealKMSClient_Decrypt_OK(t *testing.T) {
	...
	r := &realKMSClient{
		keyID: "alias/test",
		sdkDec: func(req *kmssdk.DecryptRequest) (*kmssdk.DecryptResponse, error) {
			if req.CiphertextBlob == nil {
				t.Fatal("CiphertextBlob nil")
			}
			// 验证 base64 编码正确
			raw, err := base64.StdEncoding.DecodeString(*req.CiphertextBlob)
			if err != nil {
				t.Fatalf("CiphertextBlob not base64: %v", err)
			}
			if !bytes.Equal(raw, []byte("payload-encdek")) {
				t.Errorf("encDEK mismatch")
			}
			return &kmssdk.DecryptResponse{
				Body: &kmssdk.DecryptResponseBody{
					Plaintext: strPtr(base64.StdEncoding.EncodeToString(dek)),
				},
			}, nil
		},
...
func TestParseGenerateDataKeyResponse_NilBody(t *testing.T) {
	_, _, err := parseGenerateDataKeyResponse(&kmssdk.GenerateDataKeyResponse{Body: nil})
	if err == nil || !bytes.Contains([]byte(err.Error()), []byte("empty body")) {
		t.Fatalf("want empty body err, got %v", err)
	}
}
...
func TestParseGenerateDataKeyResponse_MissingFields(t *testing.T) {
	// Plaintext nil
	_, _, err := parseGenerateDataKeyResponse(&kmssdk.GenerateDataKeyResponse{
		Body: &kmssdk.GenerateDataKeyResponseBody{CiphertextBlob: strPtr("abc")},
	})
	...
	// CiphertextBlob nil
	_, _, err = parseGenerateDataKeyResponse(&kmssdk.GenerateDataKeyResponse{
		Body: &kmssdk.GenerateDataKeyResponseBody{Plaintext: strPtr("abc")},
	})
	if err == nil || !bytes.Contains([]byte(err.Error()), []byte("missing")) {
		t.Fatalf("want missing err, got %v", err)
	}
...
func TestParseGenerateDataKeyResponse_BadBase64(t *testing.T) {
	...
	_, _, err := parseGenerateDataKeyResponse(&kmssdk.GenerateDataKeyResponse{
		Body: &kmssdk.GenerateDataKeyResponseBody{
			Plaintext:      strPtr("@@@not-base64@@@"),
			CiphertextBlob: strPtr(base64.StdEncoding.EncodeToString([]byte("ok"))),
		},
	})
	if err == nil || !bytes.Contains([]byte(err.Error()), []byte("decode Plaintext")) {
		t.Fatalf("want decode Plaintext err, got %v", err)
	}
	// CiphertextBlob invalid base64
	_, _, err = parseGenerateDataKeyResponse(&kmssdk.GenerateDataKeyResponse{
		Body: &kmssdk.GenerateDataKeyResponseBody{
			Plaintext:      strPtr(base64.StdEncoding.EncodeToString([]byte("dek"))),
			CiphertextBlob: strPtr("@@@bad@@@"),
		},
	})
	if err == nil || !bytes.Contains([]byte(err.Error()), []byte("decode CiphertextBlob")) {
		t.Fatalf("want decode CiphertextBlob err, got %v", err)
	}
}

func TestParseDecryptResponse_NilResp(t *testing.T) {
	_, err := parseDecryptResponse(nil)
	if err == nil || !bytes.Contains([]byte(err.Error()), []byte("empty body or Plaintext")) {
		t.Fatalf("want empty err, got %v", err)
	}
	_, err = parseDecryptResponse(&kmssdk.DecryptResponse{Body: nil})
	if err == nil {
		t.Fatal("want err for nil body")
	}
	_, err = parseDecryptResponse(&kmssdk.DecryptResponse{Body: &kmssdk.DecryptResponseBody{Plaintext: nil}})
	if err == nil {
		t.Fatal("want err for nil Plaintext")
	}
}

func TestParseDecryptResponse_BadBase64(t *testing.T) {
	_, err := parseDecryptResponse(&kmssdk.DecryptResponse{
		Body: &kmssdk.DecryptResponseBody{Plaintext: strPtr("@@@bad@@@")},
	})
	if err == nil || !bytes.Contains([]byte(err.Error()), []byte("decode Plaintext")) {
