...
func TestMetadata(t *testing.T) {
	c := New(Config{})
	if got := c.Name(); got != "buypass" {
		t.Errorf("Name = %q, want %q", got, "buypass")
	}
	if got := c.Tier(); got != ca.TierFreeDV {
		t.Errorf("Tier = %q, want %q", got, ca.TierFreeDV)
	}
	if !c.SupportsWildcard() {
		t.Error("SupportsWildcard = false, want true (Buypass enabled wildcards via dns-01 in 2023)")
	}
...
// TestValidityDaysIs180 pins the 180-day lifetime explicitly: this is
// the differentiator vs Let's Encrypt's 90 days and any drift would
// break the renewal scheduler's T-30d job timing.
func TestValidityDaysIs180(t *testing.T) {
	c := New(Config{})
	if got := c.ValidityDays(); got != 180 {
		t.Errorf("ValidityDays = %d, want 180", got)
	}
}

func TestDirectoryURL(t *testing.T) {
	prod := New(Config{Env: EnvProduction}).(*bpCA)
	if got := prod.directoryURL(); got != "https://api.buypass.com/acme/directory" {
		t.Errorf("production URL wrong: %s", got)
	}
	stg := New(Config{Env: EnvStaging}).(*bpCA)
	if got := stg.directoryURL(); got != "https://api.test4.buypass.no/acme/directory" {
		t.Errorf("staging URL wrong: %s", got)
	}
	def := New(Config{}).(*bpCA)
	if got := def.directoryURL(); got != "https://api.buypass.com/acme/directory" {
		t.Errorf("default URL should be production, got %s", got)
	}
}
