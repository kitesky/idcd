...
# ── 5. 等 api / cert-svc / mcp healthy ──────────────────────
step 5 "等 api + cert-svc + mcp healthcheck pass (max 60s)"
ssh "${USER}@${HOST}" 'for i in $(seq 1 30); do
  ...
  mcp=$(docker inspect -f "{{.State.Health.Status}}" idcd-mcp 2>/dev/null || echo "missing")
  echo "[$i/30] api=$api cert-svc=$cert mcp=$mcp"
  # mcp 是新加服务,允许 missing(老 compose 还没声明 mcp 时不阻塞 deploy)
  if [[ "$api" == "healthy" && "$cert" == "healthy" ]] && [[ "$mcp" == "healthy" || "$mcp" == "missing" ]]; then
    exit 0
  fi
  sleep 2
done; exit 1' || { red "api / cert-svc / mcp 30 次后仍不 healthy,看日志:"; \
    ssh "${USER}@${HOST}" 'docker compose -f /opt/idcd/docker-compose.prod.yml logs --tail=30 api cert-svc mcp'; exit 5; }

# ── 6. 烟测关键 endpoint ──────────────────────────────────────
step 6 "烟测 /health + /healthz + /v1/cert (经 api 反代) + mcp /healthz"
ssh "${USER}@${HOST}" 'set -e
  ...
  # mcp /healthz — mcp 容器没起也不阻塞 deploy,只在 stdout 标 SKIP
  if docker ps --format "{{.Names}}" | grep -q "^idcd-mcp$"; then
    echo "  mcp    /healthz:       $(curl -fsS http://127.0.0.1:8082/healthz | head -c 60)"
  else
    echo "  mcp    /healthz:       SKIP (idcd-mcp container not running)"
  fi
'
