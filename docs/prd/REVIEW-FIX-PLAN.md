...
- **文件**：`frontend/src/app/about/page.tsx:4`
...
- **文件**：`backend/apps/api/internal/handler/auth.go:299`
...
- **文件**：`backend/apps/api/internal/server/server.go:127`
...
- **文件**：`frontend/src/app/auth/login/page.tsx:55`，`frontend/src/lib/api.ts:4`
...
- **文件**：`backend/apps/api/internal/middleware/csrf.go:107`
- **修复**：`subtle.ConstantTimeCompare([]byte(headerToken), []byte(cookieToken)) == 1`

### F2-3 CSP 仅 Report-Only + 含 unsafe-inline/eval
- **文件**：`backend/apps/api/internal/middleware/security_headers.go:11`
- **修复**：
...
- **文件**：`frontend/src/components/cookie-banner.tsx:61,69`
...
- **文件**：`backend/apps/api/internal/middleware/rate_limit.go:82`，`handler/probe.go:261`
- **修复**：仅当 `r.RemoteAddr` 属于可信代理 CIDR 时才信任 `X-Forwarded-For`；使用最右侧可信 IP

### F2-10 `/metrics` 无认证暴露在公网端口
- **文件**：`backend/apps/api/internal/server/server.go:108`
...
- **文件**：`backend/apps/api/internal/middleware/cors.go:31`
